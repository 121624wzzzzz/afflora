...
def add_confirmation():
 ...
 write(HERE/'SELECTION_FROZEN.json',dict(at=now(),criterion='mean dev primary over three fixed seeds; ties favor ratio closest to 1',candidates=[.00390625,.015625,.0625,.25,1.,4.,16.],selection=[dict(architecture=list(k),ratio=r,dev_scores=dict(groups[k])) for k,r in choices.items()],jobs=selected))
 for s in selected:
