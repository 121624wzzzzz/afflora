...
def main():
 ...
 lines=['# Qwen3.5-4B Base 叠加实验','',
  '两任务、三组、同一学习率搜索和五种子确认；具体结论由下面的完整结果决定。所有原模型参数冻结，仅指定适配器训练。',
 ...
 for c in a['comparisons']:
  ...
  lines.append(f'|{c["task"]}|{LABEL[c["b"]]}|{c["mean_pp"]:+.4f}|{ds}|{ci(c["bonferroni4_ci95"])}|')
 lines+=['']
 ...
 if all(c['mean_pp']<0 for c in a['comparisons']) and all(c['bonferroni4_ci95'][0]<=0<=c['bonferroni4_ci95'][1] for c in a['comparisons']):
