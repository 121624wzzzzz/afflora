...
def main():
    data=read(HERE/'ANALYSIS.json');rows_=data['primary_contrasts']
    ...
    ax.set_title('Five-seed development pilot: additive content effects',loc='left')
    ax.grid(axis='x',alpha=.2);ax.spines[['top','right']].set_visible(False)
    fig.text(.02,.015,'95% seed-t intervals, Bonferroni family 8. 2,048 train / 200 dev; one shared LR. No held-out test claim.',fontsize=9)
    fig.tight_layout(rect=[0,.045,1,1]);(HERE/'figures').mkdir(exist_ok=True)
    for ext in ['png','pdf','svg']:fig.savefig(HERE/'figures'/f'additive_effects.{ext}',dpi=180,bbox_inches='tight')
    plt.close(fig)
