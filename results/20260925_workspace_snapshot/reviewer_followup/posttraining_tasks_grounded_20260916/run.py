...
def evaluate(model,spec,tag,limit=None):
    data=rows(HERE/f"data/{spec['task']}_{spec.get('eval_split','pilot_dev')}.jsonl")
    tokens=read(HERE/f"tokens/{spec['task']}_{spec['model']}_{spec.get('eval_split','pilot_dev')}.json")
    ...
    base_precision(model,torch.float32);model.eval();records=[];started=time.monotonic();answer_sum=eos_sum=0.;answer_n=eos_n=0
    for start in range(0,len(data),8):
        chunk=tokens[start:start+8];ids,mask,_=batch(chunk,False)
        with torch.inference_mode():
            generated=model.generate(input_ids=ids,attention_mask=mask,do_sample=False,max_new_tokens=512,
                eos_token_id=151643,pad_token_id=151643,use_cache=True)
        for r,seq in zip(data[start:start+8],generated[:,ids.shape[1]:].tolist()):
            ...
            records.append(dict(s,id=r['id'],text=text,token_ids=seq,native_eos=ended,capped=not ended and len(seq)>=512,length=len(seq)))
        # Teacher-forced answer/EOS losses on the same evaluated rows, independent
        # of whether generation could be parsed.
        for i in range(0,len(chunk),2):
            ids,mask,labels=batch(chunk[i:i+2])
            with torch.inference_mode():_,ce,gold=loss(model,ids,mask,labels)
            eos=gold==151643;answer_sum+=float(ce[~eos].sum());eos_sum+=float(ce[eos].sum());answer_n+=int((~eos).sum());eos_n+=int(eos.sum())
        ...
        print(json.dumps({'evaluation':tag,'done':len(records),'total':len(data)}),flush=True)
    summary=aggregate(records,spec['task']);summary.update(native_eos_pct=100*sum(x['native_eos'] for x in records)/len(records),
        capped_pct=100*sum(x['capped'] for x in records)/len(records),mean_generated_tokens=sum(x['length'] for x in records)/len(records),
        answer_ce=answer_sum/answer_n,eos_ce=eos_sum/eos_n,seconds=time.monotonic()-started,spec=spec,tag=tag,
...
def main():
    ...
    model,audit=build(spec);data=read(HERE/f"tokens/{spec['task']}_{spec['model']}_{spec.get('train_split','pilot_train')}.json")
    audit['preflight']=preflight(model,spec,data);audit['frozen_before']=frozen_digest(model);write(root/'INITIALIZATION.json',audit)
    if spec['arm']=='base':
        evaluate(model,spec,'baseline');write(root/'COMPLETE.json',{'at':now(),'status':'passed'});return
    ...
    for step in range(steps):
        ...
        for begin in range(0,len(selected),2):
            items=[data[i] for i in selected[begin:begin+2]];ids,mask,labels=batch(items)
    ...
    if not spec.get('smoke'):evaluate(model,spec,'final')
