...
def integrity():
 ...
 for root,h in reuse['source_manifests'].items():
  source=Path(root);manifest=source/('SEAL_MANIFEST.json' if (source/'SEAL_MANIFEST.json').exists() else 'PARTIAL_ARCHIVE_MANIFEST.json')
  assert sha(manifest)==h,root
 ...
 for j in state['jobs']:
  ...
  assert audit['numerical_policy_final_sha256']==sha(ROOT/'checkpoints'/name/'NUMERICAL_POLICY_FINAL.json')
 ...
 assert analysis['family']==16
 assert read(ROOT/'INITIAL_LOSS_REPRODUCIBILITY.json')['max_range']==0
 assert read(ROOT/'INITIAL_REFERENCES.json')['status']=='passed'
 assert len(read(ROOT/'INITIAL_REFERENCES.json')['records'])==16
 ...
 for c in analysis['comparisons']:
  ...
  for key,level in [('ci95',.975),('bonferroni16_ci95',1-.05/32)]:
...
def main():
 ...
 checks=integrity()
 for name in ['seal_qwen35_fixed_size_20260920.py','report_qwen35_fixed_size_20260920.py','configure_qwen35_fixed_20260920.py','finalize_qwen35_fixed_source_20260920.py','schedule_qwen35_fixed_20260920.py','prepare_qwen35_fixed_20260920.py','close_qwen35_fixed_20260920.py','plot_qwen35_fixed_20260920.py']:
  p=ROOT.parent/name
  if p.exists():(ROOT/'provenance'/(name+'.txt')).write_bytes(p.read_bytes())
 key=ROOT.name.split('_')[2]
