...
def main():
 ...
 for ax,field,title in [(axes[0],'bonferroni12_ci95','Paired training seeds (5 seeds)'),(axes[1],'bonferroni12_percentile_ci95','Data clusters (fixed five models)')]:
  ...
  ax.invert_yaxis();ax.set_title(title+'\nFamily-12 95% intervals',fontsize=10);ax.set_xlabel('Accuracy difference (percentage points)');ax.grid(axis='x',alpha=.18)
 ...
 lines=['# '+model_identity['repo']+' 叠加实验','',
  '两任务、三组、同一学习率搜索和五种子确认；具体结论由完整结果决定。所有原模型参数冻结，仅指定适配器训练。本轮新增每次拟合的首批重复前向/反向门禁，详见 INITIAL_REPEAT.json；通过不等于全过程跨设备确定性。',
  '', '本轮扩展的是Qwen家族内采用线性注意力与全注意力混合结构的新架构，不应计作新增独立模型家族。',
  '', f'H 为{budget["hidden_parameters"]:,}个参数；严格等参 H 与 H+E+U 均为{budget["hidden_parameters"]+budget["extra"]:,}个参数。原生输入输出权重共享={model_identity["tie_word_embeddings"]}。H 覆盖全部指定文本解码投影；E/U各rank16，E有偏置、U无偏置。视觉模块与MTP分支未参与纯文本任务。区间按三个新增尺寸共12项主要比较校正。',
 ...
 for c in a['comparisons']:
  ...
  lines.append(f'|{c["task"]}|{LABEL[c["b"]]}|{c["mean_pp"]:+.4f}|{ds}|{ci(c["bonferroni12_ci95"])}|')
 lines+=['']
 for c in a['comparisons']:lines.append(f'- {c["task"]} 相对{LABEL[c["b"]]}：{conclusion(c)}。')
 lines+=['','|任务|叠加相对对照|固定五个模型的数据重采样校正区间|','|---|---|---|']
 for c in boot['comparisons']:lines.append(f'|{c["task"]}|{LABEL[c["b"]]}|{ci(c["bonferroni12_percentile_ci95"])}|')
 if all(c['mean_pp']<0 for c in a['comparisons']) and all(c['bonferroni12_ci95'][0]<=0<=c['bonferroni12_ci95'][1] for c in a['comparisons']):
