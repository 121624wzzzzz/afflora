...
def candidate_scores(model,items,codes):
    ...
    prefixes=sorted(set(codes[:,0].tolist()));assert len(prefixes)==8
...
def verify_candidates(model,items,codes):
    ...
    return {'max_abs_joint_logprob_error':err,'argmax_equal':True,'examples':len(items),'candidate_count':len(codes),'reference':'8 independent complete prefix forwards; no cache'}
