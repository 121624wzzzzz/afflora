...
def evaluate(model,spec,tag,limit=None):
    ...
    split=spec['eval_split'];data=rows(HERE/f"data/{spec['task']}_{split}.jsonl")
    ...
    if spec['task']=='banking77':write(out/'CANDIDATE_AUDIT.json',verify_candidates(model,tokens[:2],codes))
    bs=8 if spec['task']=='banking77' else 16
    for start in range(0,len(data),bs):
        chunk=tokens[start:start+bs]
        if spec['task']=='banking77':
            values=candidate_scores(model,chunk,codes).tolist()
            for r,scores in zip(data[start:start+bs],values):
                chosen=max(range(77),key=lambda j:scores[j]);pred=f'{chosen:02d}'
                records.append({'id':r['id'],'predicted_code':pred,'gold_code':r['target'],
                    'label_logprobs':scores,'content_correct':pred==r['target']})
        else:
    ...
    if spec['task']=='e2e_clean':summary.update(native_eos_pct=100*sum(x['native_eos'] for x in records)/len(records),
...
def main():
    ...
    if spec.get('smoke'):
        evaluate(model,dict(spec,eval_split='dev'),'smoke_dev',limit=8)
    else:
        for split in ['dev','test']:evaluate(model,dict(spec,eval_split=split),split)
