...
def main():
    ...
    for name in ['SMOKE_COMPLETE.json','FORMAL_COMPLETE.json','EXPERIMENT_COMPLETE.json','RESULT_AUDIT.json','PARAMETER_AUDIT.json','SCORER_TESTS.json']:
    ...
    assert read(HERE/'RESULT_AUDIT.json')['responses']==178208
    ...
    write(HERE/'FINAL_AUDIT.json',{'at':now(),'status':'passed','model_files_rehashed':verified,
        'formal_jobs':64,'smoke_jobs':4,'responses_verified':178208,
