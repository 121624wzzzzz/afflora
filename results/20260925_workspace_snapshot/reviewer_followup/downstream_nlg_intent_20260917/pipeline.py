...
def make_jobs():
    smoke=[];jobs=[]
    for m in MODELS:
        for task,arm in [('banking77','hidden_both'),('e2e_clean','hidden_budget')]:
            smoke.append(dict(name=f'smoke_{task}_{m}_{arm}',model=m,task=task,arm=arm,seed=8099,lr=2e-4,smoke=True))
    for task in TASKS:
        for m in MODELS:jobs.append(dict(name=f'{task}_{m}_base',task=task,model=m,arm='base',seed=8100,lr=2e-4))
