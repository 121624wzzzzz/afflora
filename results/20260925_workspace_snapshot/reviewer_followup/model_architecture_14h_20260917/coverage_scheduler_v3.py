"""Coverage-priority supervisor amendment; all scientific job specs stay frozen."""
...
def main():
 ...
 for rel,h in read(ROOT/'SCHEDULER_V3_SOURCE.json')['files'].items():assert sha(ROOT/rel)==h,rel
 state=read(ROOT/'STATE_HANDOFF_PRE_V3.json');proof=read(ROOT/'HANDOFF_PROCESS_PROOF_V3.json')
 ...
 effective_priorities=read(ROOT/'COVERAGE_AMENDMENT_V3.json')['effective_priorities']
 ...
 emit({'coverage_supervisor':'v3','adopted_workers':len(running),'memory_mib':MEMORY_MIB,'scientific_specs_changed':False})
 while True:
  ...
  if now()<launch_stop:
   ...
   ordered=sorted(jobs.items(),key=lambda x:(effective_priorities[x[0]],x[1]['spec']['seed'],x[1]['spec']['task'],x[1]['spec']['model'],x[0]))
   for gpu in sorted(set(free)-occupied):
    ...
    job.update(status='running',started_at=now().isoformat(),gpu=gpu,pid=proc.pid,admission_free_mib=free[gpu],admission_required_mib=MEMORY_MIB[spec['model']],admission_effective_priority=effective_priorities[key])
  ...
  state={'at':now().isoformat(),'scheduler_version':3,'phases':phases,'halted_phases':sorted(halt),'jobs':jobs,'counts':{s:sum(r['status']==s for r in jobs.values()) for s in sorted({r['status'] for r in jobs.values()})}}
  ...
  time.sleep(10)
 write(ROOT/'SCHEDULER_COMPLETE.json',{'at':now().isoformat(),'status':'finished','scheduler_version':3,'counts':state['counts'],'halted_phases':sorted(halt),'all_planned_jobs_completed':all(r['status']=='passed' for r in jobs.values())})
