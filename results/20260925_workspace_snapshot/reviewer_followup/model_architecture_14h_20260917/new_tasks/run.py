...
def evaluate(model,spec,tag,limit=None):
    ...
    if task=='trec50':
        codes=read(HERE/f"tokens/{spec['model']}_codes.json")
    ...
    for start in range(0,len(data),bs):
        ...
        if task=='trec50':
            ...
            for r,scores in zip(data[start:start+bs],values):
                chosen=max(range(50),key=lambda j:scores[j]);pred=f'{chosen:02d}'
                records.append({'id':r['id'],'predicted_code':pred,'gold_code':r['target'],'label_logprobs':scores,'content_correct':pred==r['target']})
        else:
    ...
    if task=='squad2':summary.update(native_eos_pct=100*sum(x['native_eos'] for x in records)/len(records),capped_pct=100*sum(x['capped'] for x in records)/len(records),mean_generated_tokens=sum(x['length'] for x in records)/len(records))
