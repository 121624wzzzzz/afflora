...
sp=importlib.util.spec_from_file_location("official_squad",HERE/"raw/squad/evaluate-v2.0.py");official_squad=importlib.util.module_from_spec(sp);sp.loader.exec_module(official_squad)
...
def main():
    ...
    for tag in (['smoke_dev'] if spec['smoke'] else ['dev','test']):
        ...
        if task=='trec50':
            ca=read(p/'CANDIDATE_AUDIT.json');assert ca['argmax_equal'] and ca['max_abs_joint_logprob_error']<2e-4 and ca['candidate_count']==50
        for r,g in zip(rs,data):
            if task=='trec50':
                values=r['label_logprobs'];assert len(values)==50 and all(math.isfinite(v) for v in values)
                pred=f'{max(range(50),key=lambda i:values[i]):02d}'
                ...
                if pred is not None:
                    golds=[x for x in g['answers'] if official_squad.normalize_answer(x)] or ['']
                    assert r['em']==max(official_squad.compute_exact(x,pred) for x in golds)
                    assert abs(r['f1']-max(official_squad.compute_f1(x,pred) for x in golds))<1e-12
                    official+=1
                else:assert r['em']==r['f1']==0
            checked+=1
        got=aggregate(rs,task);assert got=={k:s[k] for k in got};summaries[tag]=sha(p/'SUMMARY.json')
    write(HERE/'audits'/f'{args.name}.json',{'at':now(),'status':'passed','name':args.name,'responses':checked,'official_squad_predictions':official,'parameter_scope':budget[arm],'summary_sha256':summaries})
