...
for file in ['MODEL_IDENTITY_AUDIT.json','DATA_AND_REUSE_AUDIT.json','SCORER_TESTS.json','INDEPENDENT_DATA_AUDIT.json']:assert read(HERE/file)['status']=='passed'
files={str(p.relative_to(HERE)):sha(p) for folder in ['data','tokens','raw'] for p in (HERE/folder).rglob('*') if p.is_file() and '__pycache__' not in p.parts}
write(HERE/'DATA_FROZEN.json',{'at':now(),'files':files})
code=list(HERE.glob('*.py'))+list((HERE/'source').glob('*.py'))+[HERE/x for x in ['PROTOCOL.md','models.json','BUDGET_PLAN.json','FORMAL_JOBS.json','SMOKE_JOBS.json','INVENTORY.json','SOURCE_REUSE.json','MODEL_IDENTITY_AUDIT.json','DATA_SOURCES.json','INDEPENDENT_DATA_AUDIT.json']]
