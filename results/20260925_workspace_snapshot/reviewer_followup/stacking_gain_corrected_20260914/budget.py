...
def add_hidden_budget(model, base_rank, seed):
    ...
    for kind in ("q_proj", "k_proj"):
        ...
        costs = {layer.in_features + layer.out_features for _, layer in candidates}
        assert len(costs) == 1
        cost = costs.pop()
        count = min(len(candidates), (target - spent) // cost) if kind == "q_proj" else math.ceil((target - spent) / cost)
