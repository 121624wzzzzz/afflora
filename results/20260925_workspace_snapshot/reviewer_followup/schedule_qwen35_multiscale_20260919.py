"""One finite GPU queue across three immutable, separately audited size studies."""
...
def process(root,script,log,args=(),gpu=None):
 env=dict(os.environ,PYTHONDONTWRITEBYTECODE='1',OMP_NUM_THREADS='4',OPENBLAS_NUM_THREADS='2',TOKENIZERS_PARALLELISM='false',TORCHINDUCTOR_COMPILE_THREADS='1')
...
def main():
 ...
 for key in KEYS:
  p=PARENT/f'qwen35_{key}_20260919';assert not (p/'STATE.json').exists()
 ...
 while True:
  ...
  if not failed:
   ...
   busy.update(gpu for gpu,key in [(7,'08b'),(1,'2b'),(2,'9b')] if studies[key]['phase']=='waiting_ready')
   for gpu in range(8):
    if gpu in busy or free.get(gpu,0)<69632:continue
    picked=None
    ...
    j.update(state='running',gpu=gpu,pid=proc.pid,started_at=now());active[key+':'+name]=(proc,key,j,f);event('started',model=key,name=name,gpu=gpu,pid=proc.pid)
  states={}
  for key,s in studies.items():
  ...
  write(MASTER/'STATE.json',dict(started_at=start,at=now(),failed=failed,studies=states))
