"""Download public Google ModelScope distribution; verify against pinned Google HF identities."""
import concurrent.futures,hashlib,json,time,math,shutil
from pathlib import Path
import requests

ROOT=Path(__file__).resolve().parents[2]
STUDY=ROOT/'lora/reviewer_followup/gemma_transfer_20260918'
MODEL=ROOT/'models/Gemma-2-9B-Base'
REV='33c193028431c2fde6c6e51f29e6f17b60cbfac6'
def session():
...
def main():
    ...
    s=session();r=s.get(f'https://huggingface.co/api/models/google/gemma-2-9b/revision/{REV}',params={'blobs':'true'},timeout=40);r.raise_for_status();official=r.json();assert official['sha']==REV
    r=s.get('https://www.modelscope.cn/api/v1/models/google/gemma-2-9b/repo/files',params={'Revision':'master','Recursive':'true'},timeout=40);r.raise_for_status();mirror=r.json();assert mirror['Success']
    write(STUDY/'provenance/google_gemma_2_9b_official_metadata.json',official)
    write(STUDY/'provenance/google_gemma_2_9b_public_distribution_metadata.json',mirror)
    index={r['Path']:r for r in mirror['Data']['Files'] if r['Type']=='blob'}
    names={'config.json','generation_config.json','model.safetensors.index.json','special_tokens_map.json','tokenizer.json','tokenizer.model','tokenizer_config.json'}
    records=[r for r in official['siblings'] if r['rfilename'] in names or r['rfilename'].endswith('.safetensors')]
    assert len([r for r in records if r['rfilename'].endswith('.safetensors')])==8
    for r in records:
    ...
    def download(r):
        ...
        url=f"https://www.modelscope.cn/models/google/gemma-2-9b/resolve/{m['Revision']}/{fn}"
    ...
    resolved=AutoConfig.from_pretrained(MODEL,local_files_only=True)
    result={'status':'passed','repo':'google/gemma-2-9b','revision':REV,'path':str(MODEL),'training_stage':'pretrained_base',
            'distribution':'public google/gemma-2-9b on ModelScope; every runtime file verified against official Google HF revision',
            'files':{str(MODEL/k):v for k,v in checked.items()},'hidden_size':cfg['hidden_size'],'tie_word_embeddings':resolved.tie_word_embeddings,
            'tying_declaration':'Gemma2Config default; checked again by actual parameter pointer at runtime',
            'bos_token_id':cfg['bos_token_id'],'eos_token_id':cfg['eos_token_id'],'pad_token_id':cfg['pad_token_id']}
    write(STUDY/'models.json',{'gemma2_9b_base':result});write(STUDY/'MODEL_IDENTITY_AUDIT.json',result)
