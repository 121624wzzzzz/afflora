...
STUDY=ROOT/'lora/reviewer_followup/qwen35_2b_20260919'
MODEL=ROOT/'models/Qwen3.5-2B-Base'
REV='b1485b2fa6dfa1287294f269f5fb618e03d52d7c'
def session():
...
def main():
    ...
    s=session();r=s.get(f'https://huggingface.co/api/models/Qwen/Qwen3.5-2B-Base/revision/{REV}',params={'blobs':'true'},timeout=40);r.raise_for_status();official=r.json();assert official['sha']==REV
    r=s.get('https://www.modelscope.cn/api/v1/models/Qwen/Qwen3.5-2B-Base/repo/files',params={'Revision':'master','Recursive':'true'},timeout=40);r.raise_for_status();mirror=r.json();assert mirror['Success']
    write(STUDY/'provenance/qwen35_2b_official_metadata.json',official)
    write(STUDY/'provenance/qwen35_2b_public_distribution_metadata.json',mirror)
    ...
    def download(r):
        ...
        url=f"https://www.modelscope.cn/models/Qwen/Qwen3.5-2B-Base/resolve/{m['Revision']}/{fn}"
    ...
    result={'status':'passed','repo':'Qwen/Qwen3.5-2B-Base','revision':REV,'path':str(MODEL),'training_stage':'pretrained_base',
            'distribution':'public Qwen/Qwen3.5-2B-Base on ModelScope; every runtime file verified against official Qwen HF revision',
    ...
    write(STUDY/'models.json',{'qwen35_2b_base':result});write(STUDY/'MODEL_IDENTITY_AUDIT.json',result)
