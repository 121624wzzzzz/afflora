...
def build(spec):
    ...
    if arm in ['hidden_r8','small_q']:
        if arm=='hidden_r8':
            ...
            middle=len(model.model.layers)//2
            projection=model.model.layers[middle].self_attn.q_proj
            budget=33*model.config.hidden_size
            rank=budget//(projection.in_features+projection.out_features)
            assert rank>=1
            config=LoraConfig(task_type=TaskType.CAUSAL_LM,r=rank,lora_alpha=2*rank,
                lora_dropout=.05,target_modules=['q_proj'],layers_to_transform=[middle],bias='none')
        model=get_peft_model(model,config).get_base_model()
    ...
    assert all(('.lora_A.' in n or '.lora_B.' in n) if arm in ['hidden_r8','small_q'] else n.startswith(PREFIX) for n in trainable)
    assert any('lora_' in n for n,p in model.named_parameters())==(arm in ['hidden_r8','small_q'])
    count=sum(p.numel() for p in trainable.values());d=model.config.hidden_size
    if arm in ['hidden_r8','small_q']:
        expected=(8 if arm=='hidden_r8' else rank)*sum(module.in_features+module.out_features for module in model.modules() if isinstance(module,BaseTunerLayer))
    ...
    audit={'arm':arm,'trainable_parameters':count,'trainable_names':list(trainable),'base_parameter_count':sum(p.numel() for n,p in model.named_parameters() if not is_adapter_name(n)),
        'hidden_lora_installed':arm in ['hidden_r8','small_q'],'adapter_init_sha256':digest(adapter_state(model)),
    ...
    if arm=='small_q':
        assert len([m for m in model.modules() if isinstance(m,BaseTunerLayer)])==1
        audit.update(target_layer=middle,target_projection='q_proj',rank=rank,alpha=2*rank,
                     input_alora_budget=budget,budget_fraction=count/budget)
