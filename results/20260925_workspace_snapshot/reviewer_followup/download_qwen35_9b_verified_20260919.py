...
STUDY=ROOT/'lora/reviewer_followup/qwen35_9b_20260919'
MODEL=ROOT/'models/Qwen3.5-9B-Base'
REV='68c46c4b3498877f3ef123c856ecfde50c39f404'
def session():
...
def main():
    ...
    s=session();r=s.get(f'https://huggingface.co/api/models/Qwen/Qwen3.5-9B-Base/revision/{REV}',params={'blobs':'true'},timeout=40);r.raise_for_status();official=r.json();assert official['sha']==REV
    r=s.get('https://www.modelscope.cn/api/v1/models/Qwen/Qwen3.5-9B-Base/repo/files',params={'Revision':'master','Recursive':'true'},timeout=40);r.raise_for_status();mirror=r.json();assert mirror['Success']
    write(STUDY/'provenance/qwen35_9b_official_metadata.json',official)
    write(STUDY/'provenance/qwen35_9b_public_distribution_metadata.json',mirror)
    ...
    assert len([r for r in records if r['rfilename'].endswith('.safetensors')])==4
    for r in records:
    ...
    def download(r):
        ...
        url=f"https://www.modelscope.cn/models/Qwen/Qwen3.5-9B-Base/resolve/{m['Revision']}/{fn}"
        ...
        if planpath.exists() or (fn.endswith('.safetensors') and r['size']-start>256*1024*1024):
            ...
            def fetch_range(pair):
                ...
                for attempt in range(20):
                    ...
                    try:
                        response=session().get(url,headers={'Range':f'bytes={lo+offset}-{hi}'},stream=True,timeout=(30,60))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ranges_pool:
    ...
    result={'status':'passed','repo':'Qwen/Qwen3.5-9B-Base','revision':REV,'path':str(MODEL),'training_stage':'pretrained_base',
            'distribution':'public Qwen/Qwen3.5-9B-Base on ModelScope; every runtime file verified against official Qwen HF revision',
    ...
    write(STUDY/'models.json',{'qwen35_9b_base':result});write(STUDY/'MODEL_IDENTITY_AUDIT.json',result)
