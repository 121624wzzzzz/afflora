#!/usr/bin/env python
"""A-LoRA rank screening at low/high hidden-LoRA capacity anchors."""
...
HERE = ROOT / "reviewer_followup/llama_affine_rank_sweep"
...
class Job:
    ...
    def placement(self) -> str:
        return "lmhead" if self.model_alias == "llama31_8b" else "mergeable"

    @property
    def name(self) -> str:
        return (
            f"{self.model_alias}_{self.placement}_ar{self.affine_rank}_s1_"
            f"hr{self.hidden_rank}_seed{SEED}"
        )
    ...
    def reused(self) -> bool:
        return self.hidden_rank == 4 and self.affine_rank == 16

    @property
    def checkpoint_dir(self) -> Path:
        return (EXISTING if self.reused else CHECKPOINTS) / self.name


JOBS = [
    Job(alias, hidden_rank, affine_rank)
    for alias in MODELS
...
def initial_state() -> dict[str, Any]:
    if STATE_FILE.exists():
        ...
        # Keep state only for the active matrix. This also cleanly handles a
        # narrowed sweep after a scheduler was stopped before any checkpoint
        # completed.
        state["jobs"] = {
            job.name: previous.get(job.name, {"status": "pending", "attempts": 0})
            for job in JOBS
        }
        return state
    return {
        "schema_version": 1, "started_at": now(), "updated_at": now(),
        "phase": "waiting_for_hidden_rank", "jobs": {
            job.name: {"status": "pending", "attempts": 0} for job in JOBS
        },
    }
...
def checkpoint_complete(job: Job) -> bool:
    required = (
        ...
        job.checkpoint_dir / "affine_vocab_adapter.safetensors",
        job.checkpoint_dir / "affine_vocab_config.json",
...
def eval_complete(job: Job) -> bool:
    ...
    try:
        ...
        return (
            report["num_samples"] == 499 and report["answer_tokens"] > 0
...
def hidden_loss(model_alias: str, hidden_rank: int) -> float | None:
    path = (
        HIDDEN_SWEEP / "outputs/metamath_loss" /
        f"{model_alias}_hidden_hr{hidden_rank}_seed{SEED}.json"
    )
    if not path.is_file():
        return None
    try:
        return float(json.loads(path.read_text(encoding="utf-8"))["loss"])
...
def train_command(job: Job) -> list[str]:
    ...
    cmd = [
        ...
        "--model-path", str(job.model_path), "--train-data", str(TRAIN),
        "--output-dir", str(job.checkpoint_dir), "--variant", variant,
        ...
        "--affine-alpha", str(job.affine_rank), "--max-seq-len", "1024",
        ...
        "--seed", str(SEED), "--master-dtype", "fp32", "--base-dtype", "bf16",
...
def train_one(job: Job, gpu: int) -> bool:
    if checkpoint_complete(job):
        update_job(job.name, status="trained", gpu=None, reused=job.reused)
        event("training_skip_complete", job=job.name, reused=job.reused)
        return True
    for attempt in (1, 2):
...
def eval_one(job: Job, gpu: int) -> bool:
    ...
    for attempt, batch in enumerate(batches, 1):
        ...
        cmd = [
            ...
            "--output-file", str(eval_path(job)), "--batch-size", str(batch),
...
def write_summary_locked() -> None:
    ...
    lines = [
        "# Llama A-LoRA rank screening", "", f"Updated: {now()}", "",
    ...
    for job in JOBS:
        base = hidden_loss(job.model_alias, job.hidden_rank)
        loss, ppl = result_for(job)
        delta = None if base is None or loss is None else loss - base
        fmt = lambda value: "-" if value is None else f"{value:.8f}"
        lines.append(
            f"| {job.model_alias} | {job.placement} | {job.hidden_rank} | "
            f"{job.affine_rank} | {fmt(base)} | {fmt(loss)} | {fmt(delta)} | "
            f"{fmt(ppl)} | {'reused' if job.reused else 'new'} |"
        )
...
def main() -> None:
    ...
    atomic_json(STATE_FILE, STATE)
    event("phase", phase="waiting_for_hidden_rank")
    while tmux_exists("alora_review_llama_hidden_rank"):
        time.sleep(5)
    missing = [
        f"{alias}:hr{rank}" for alias in MODELS for rank in HIDDEN_RANKS
        if hidden_loss(alias, rank) is None
    ]
    if missing:
        with STATE_LOCK:
            STATE["phase"] = "blocked_missing_hidden_baseline"
            atomic_json(STATE_FILE, STATE)
        raise SystemExit(f"Missing hidden baselines: {missing}")

    with STATE_LOCK:
        STATE["phase"] = "mixed_training_and_dev_evaluation"
        STATE["updated_at"] = now()
        atomic_json(STATE_FILE, STATE)
        write_summary_locked()
    ...
    def enqueue_eval(job: Job) -> None:
        with QUEUE_LOCK:
            if job.name not in eval_enqueued:
    ...
    for job in JOBS:
        if not checkpoint_complete(job):
            ...
            update_job(job.name, status="complete", gpu=None, reused=job.reused)
        else:
            update_job(job.name, status="trained", gpu=None, reused=job.reused)
    for job in JOBS:
