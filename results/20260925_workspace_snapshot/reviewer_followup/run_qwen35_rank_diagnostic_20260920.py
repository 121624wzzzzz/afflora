"""Run a fixed eight-fit rank diagnostic with paired provenance and per-fit audits."""
import os,json,hashlib,subprocess,time,datetime,traceback,statistics
from pathlib import Path
P=Path(__file__).resolve().parent;R=P/'qwen35_2b_rank_diagnostic_20260920';BASE=P/'qwen35_fixed_2b_20260920';PY='/home/wz/anaconda3/envs/qwen35_t26/bin/python'
...
def report(results):
 lines=['# Rank 分配诊断阶段结果','', '事后 WikiSQL 开发集诊断；配对种子，严格等参。每行保留全部已完成配置，不进行结果筛选。','', '|分配|LR|种子|普通|原等参|新等参|原叠加（同 LR）|','|---|---:|---:|---:|---:|---:|---:|']
 for r in sorted(results,key=lambda x:(x['variant'],x['lr'],x['seed'])):
  s=r['scores'];lines.append(f"|{r['variant']}|{r['lr']:g}|{r['seed']}|{s['hidden']:.4f}|{s['hidden_budget']:.4f}|{s['new_budget']:.4f}|{s['hidden_both']:.4f}|")
 lines+=['','这些配置使用统一组学习率。降低 E/U 学习率的独立诊断见 ../qwen35_2b_boundary_lr_20260920/REPORT_ZH.md；不能把原高 LR 叠加失稳当作该方法充分调参后的表现。两种子不作显著性或普遍优势声明。']
...
def main():
 ...
 while True:
  ...
  if not any(j['state']=='failed' for j in jobs) and disk>=80:
   ...
   for j in jobs:
    if j['state']!='pending' or not available or len(active)>=4:continue
