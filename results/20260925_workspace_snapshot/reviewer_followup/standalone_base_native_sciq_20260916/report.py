...
def main():
    ...
    for m,o in r['models'].items():
        ...
        lines.append('| '+LABELS[m]+' | '+' | '.join(f'{v:.3f}%' for v in vals)+' |')
    lines += ['', '“从 Base 开始”与“冻结原始 Base 参数”不冲突：训练的适配器改变指定位置的有效计算。本轮使用固定文本补全提示、答案字母和原生 EOS，Base 的词表能区分该停止目标；结果仍依赖输入协议，不能把任务收益全部解释为新增知识。',
        '', '## 纠正的 Base 监督协议','',
        ...
        '', '## 冻结与复用审计','',
        f"全部 {audit['training_jobs']} 次训练（6烟测、18验证调参、30确认）通过原始参数逐位不变、每步冻结参数无梯度及优化器白名单检查。A-LoRA 实验没有隐藏 LoRA，内部 LoRA 对照没有边界适配器。所有 adapter 为 FP32 且从零残差开始，确认种子5002–5006。",
