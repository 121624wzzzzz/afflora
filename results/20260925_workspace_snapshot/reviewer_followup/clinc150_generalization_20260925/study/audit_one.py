...
def main():
    ...
    for tag in (['smoke_dev'] if spec['smoke'] else spec.get('eval_splits',['dev','test'])):
        ...
        for r,g in zip(rs,data):
            ...
            independent.append(pred if valid else '__invalid__')
            assert r['content_correct']==(independent[-1]==g['target'])
        assert abs(s['primary']-100*accuracy_score([r['target'] for r in data],independent))<1e-9
        assert abs(s['macro_f1']-100*f1_score([r['target'] for r in data],independent,labels=sorted({r['target'] for r in data}),average='macro',zero_division=0))<1e-9
