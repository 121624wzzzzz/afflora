...
STUDY=ROOT/'lora/reviewer_followup/qwen35_transfer_20260919'
MODEL=ROOT/'models/Qwen3.5-4B-Base'
...
def main():
    ...
    s=session();r=s.get(f'https://huggingface.co/api/models/Qwen/Qwen3.5-4B-Base/revision/{REV}',params={'blobs':'true'},timeout=40);r.raise_for_status();official=r.json();assert official['sha']==REV
    r=s.get('https://www.modelscope.cn/api/v1/models/Qwen/Qwen3.5-4B-Base/repo/files',params={'Revision':'master','Recursive':'true'},timeout=40);r.raise_for_status();mirror=r.json();assert mirror['Success']
    write(STUDY/'provenance/qwen35_4b_official_metadata.json',official)
    write(STUDY/'provenance/qwen35_4b_public_distribution_metadata.json',mirror)
    ...
    assert len([r for r in records if r['rfilename'].endswith('.safetensors')])==2
    for r in records:
    ...
    def download(r):
        ...
        url=f"https://www.modelscope.cn/models/Qwen/Qwen3.5-4B-Base/resolve/{m['Revision']}/{fn}"
    ...
    result={'status':'passed','repo':'Qwen/Qwen3.5-4B-Base','revision':REV,'path':str(MODEL),'training_stage':'pretrained_base',
            'distribution':'public Qwen/Qwen3.5-4B-Base on ModelScope; every runtime file verified against official Qwen HF revision',
    ...
    write(STUDY/'models.json',{'qwen35_4b_base':result});write(STUDY/'MODEL_IDENTITY_AUDIT.json',result)
