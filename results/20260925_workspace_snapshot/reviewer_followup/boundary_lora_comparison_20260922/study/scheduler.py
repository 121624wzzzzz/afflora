...
active={};failed=[];done=[];passed=set();blocked=set();confirmation_added=False
for s in all_jobs:
...
def required(s):
 return 60000 if s['model'] in ['qwen25_7b_base','qwen3_8b_base','llama31_8b_base'] else (38000 if s['model']=='qwen3_4b_base' else (20000 if '15b' in s['model'] else 14000))
def status(stage):
 write(HERE/'STATE.json',dict(at=now(),stage=stage,done=done,failed=failed,pending=[s['name'] for s in pending],blocked_architectures=[list(k) for k in blocked],active={str(g):dict(name=v['spec']['name'],pid=v['p'].pid,phase=v['phase']) for g,v in active.items()},reused=read(HERE/'READY.json')['reused'],confirmation_added=confirmation_added,free_disk_GiB=shutil.disk_usage(HERE).free/2**30))
def add_confirmation():
 ...
 write(HERE/'SELECTION_FROZEN.json',dict(at=now(),criterion='mean dev primary over two fixed seeds; ties favor ratio closest to 1',candidates=[.25,1.,4.],selection=[dict(architecture=list(k),ratio=r,dev_scores=dict(groups[k])) for k,r in choices.items()],jobs=selected))
 for s in selected:
...
try:
 while pending or active or not confirmation_added:
  ...
  if shutil.disk_usage(HERE).free<20*2**30:
  ...
  for gpu,mb in sorted(mem.items(),key=lambda x:-x[1]):
   ...
   def priority(s):return (0 if not s['smoke'] and s['stage']=='common' and s['method']=='vocab' and s['rank']==1 else 1 if s['smoke'] else 2 if s['stage']=='tuning' else 3,-required(s))
