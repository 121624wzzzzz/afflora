...
from modeling import build,adapter_state,digest,frozen_digest,batch,forward_selected,PREFIX
...
def main():
    ...
    model.eval();ids,mask,lengths=batch(data[:3],meta,train=True);aff=model.standalone_affine
    with torch.inference_mode():
        active=forward_selected(model,ids,mask,lengths,train=True);scales=(aff.scale,aff.bias_scale)
        aff.scale=0;aff.bias_scale=0;inactive=forward_selected(model,ids,mask,lengths,train=True)
        aff.scale,aff.bias_scale=scales
    ...
    if spec['phase']=='smoke':
        ...
        with torch.inference_mode():
            ...
            full=F.cross_entropy(all_logits.reshape(-1,got.shape[-1]),targets.reshape(-1));loss_error=float(abs(optimized-full));assert loss_error<1e-5
        torch.save({'ids':ids.cpu(),'mask':mask.cpu(),'lengths':lengths.cpu(),'logits':got.cpu(),'full_logit_error':error,
            'masked_loss_error':loss_error,'tensor_sha256':digest(adapter_state(model))},cp/'SMOKE_FORWARD.pt')
