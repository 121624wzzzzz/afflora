...
def run_phase(phase):
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        for future in as_completed([pool.submit(worker,g) for g in range(8)]):future.result()
    write(HERE/f'{phase.upper()}_COMPLETE.json',{'at':now(),'jobs':len(jobs)})

def freeze():
    assert read(HERE/'SMOKE_COMPLETE.json')['jobs']==4;init=[]
    ...
    for model in read(HERE/'models.json'):
        ...
        assert len({r['linear_init_sha256'] for r in records})==1
        assert len({r['frozen_parameters_before_sha256'] for r in records})==1
    write(HERE/'PREFLIGHT.json',{'at':now(),'status':'passed','smoke_jobs':4,'single_side_only':True})
    files=list(HERE.glob('*.py'))+[HERE/'DESIGN.md',HERE/'REUSE_AUDIT.json']
    files += [HERE/p for p in read(HERE/'REUSE_AUDIT.json')['copied_source_sha256']]
    write(HERE/'FROZEN_PROTOCOL.json',{'at':now(),'sha256':{str(p):sha(p) for p in sorted(set(files))},'confirmation_seeds':CONFIRMATION_SEEDS,'primary_family_size':8})

def select():
    assert read(HERE/'TUNING_COMPLETE.json')['jobs']==12;result={}
    for model in read(HERE/'models.json'):
        result[model]={}
        for arm in ARMS:
            candidates=[]
