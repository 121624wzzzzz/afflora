...
def main():
    ...
    copies=read(HERE/'REUSE_AUDIT.json');old=Path(copies['source_root']);seal=read(old/'ARTIFACT_MANIFEST.json')['sha256']
    for p,h in seal.items():assert sha(p)==h,p
    tokenizers={m:AutoTokenizer.from_pretrained(c['path'],local_files_only=True) for m,c in read(HERE/'models.json').items()}
    count=gens=frozen=near_flips=gen_flips=0;frozen_hashes={};base_replays={}
    for phase,njobs in [('smoke',4),('tuning',12),('confirmation',20),('reference',2)]:
        ...
        for s in specs(phase):
            ...
            if phase!='reference':
                ...
                assert set(state)==set(init['trainable_names']) and all(k.startswith('standalone_affine.') for k in state)
                ...
                assert not init['hidden_lora_installed'] and train['optimizer_whitelist_verified']
                ...
                assert train['hook_counts'][s['arm']]>0 and train['hook_counts'][{'input':'output','output':'input'}[s['arm']]]==0
                frozen+=1
            if phase=='smoke':
            ...
            tok=tokenizers[s['model']];total_valid=total_correct=0
            for g in gg:
                ...
                if not g['ambiguous_gold']:total_valid+=valid;total_correct+=correct
            ...
            if phase=='reference':
                # Compare to independently disabled-adapter reference from the sealed mechanism audit.
                previous=HERE.parent/'stacking_sciq_mechanism_audit_20260915';manifest=read(previous/'ARTIFACT_MANIFEST.json')['sha256']
                path=previous/'ablations'/f"confirmation_{s['model']}_both_sd2002_lr0.0002"/'scores.npz'
                assert sha(path)==manifest[str(path)]
                with np.load(path) as oldscores:
                    error=float(np.max(np.abs(ll-oldscores['unadapted_base'])));flips=int(np.sum(ll.argmax(1)!=oldscores['unadapted_base'].argmax(1)))
                assert error<5e-4 and flips==0
                base_replays[s['model']]={'max_abs_label_logit_error':error,'prediction_flips':flips,'rows':len(rr)}
    assert all(len(h)==1 for h in frozen_hashes.values())
    assert count==100000 and gens==22000 and frozen==36
    write(HERE/'FINAL_AUDIT.json',{'at':now(),'status':'passed','training_jobs':frozen,'all_frozen_weights_bitwise_unchanged':True,
        ...
        'near_tie_prediction_flips':near_flips,'generation_first_token_shape_disagreements':gen_flips,'independent_base_replays':base_replays,
        'original_sealed_files_rechecked':len(seal),'single_side_trainable_whitelist_verified':True})
