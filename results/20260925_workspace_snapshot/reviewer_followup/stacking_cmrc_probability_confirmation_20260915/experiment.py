...
ARMS=['none','both','hidden_budget']
MODELS={k:v['path'] for k,v in read(CHAT/'manifest.json')['models'].items()}

def jobs(smoke=False):
    return [{'name':('smoke_' if smoke else '')+f'{m}_{a}_hr8_sd{s}','model':m,'arm':a,'seed':s,'smoke':smoke,
        'checkpoint':str(HERE/'checkpoints'/(('smoke_' if smoke else '')+f'{m}_{a}_hr8_sd{s}'))}
        for s in ([1001] if smoke else SEEDS) for m in MODELS for a in ARMS]

def variant(j):return {'none':'hidden_lora','both':'affine_input_lm_head_plus_hidden_lora','hidden_budget':'hidden_lora'}[j['arm']]
