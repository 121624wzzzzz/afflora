...
def get(task,model,arm,seed):
    path=HERE/'evaluations'/name(task,model,arm,seed)/'final'
...
def main():
    assert read(HERE/'ADAPTER_PILOT_COMPLETE.json')['status']=='passed'
    ...
    for task in TASKS:
        dev=rows(HERE/f'data/{task}_pilot_dev.jsonl');ids=[r['id'] for r in dev]
        if task=='toolace':
            mapping={r['id']:i for i,g in enumerate(components(rows(HERE/'data/toolace_dev.jsonl'))) for r in g}
        ...
        for model in MODELS:
            baseline=read(HERE/'evaluations'/f'pilot_{task}_{model}_base'/'baseline/SUMMARY.json')
            tables.append({'task':task,'model':model,'arm':'base','seed_count':0,'primary':baseline['primary'],
                ...
                'answer_ce':baseline['answer_ce'],'eos_ce':baseline['eos_ce']})
            values={};records={}
            for arm in ARMS:
                ...
                for metric in ['primary','text_micro_f1','json_valid','strict_json','schema_valid','native_eos_pct','capped_pct','answer_ce','eos_ce']:
    ...
    write(HERE/'ANALYSIS.json',{'at':now(),'scope':'five-seed, one-LR small-data development pilot','tables':tables,'primary_contrasts':contrasts,'all_pairing_and_budget_checks':'passed'})
    lines=['# 新后训练任务：五种子受控预实验','',
