...
def main():
    ...
    torch.set_num_threads(4);set_seed(0);j=next(x for x in jobs(a.smoke) if x['name']==a.name)
    marker=read(Path(j['checkpoint'])/'TRAIN_COMPLETE.json');hashes=checkpoint_hashes(j['checkpoint'])
    assert hashes==marker['checkpoint_hashes']
    if not a.smoke:
        manifest=read(HERE/'manifest.json')
        for path,h in manifest['local_sha256'].items():assert sha(path)==h,path
    out=HERE/('smoke_outputs' if a.smoke else 'outputs')/a.name;out.mkdir(parents=True,exist_ok=True)
    identity={'job':j,'checkpoint_hashes':hashes,'protocol_sha256':sha(HERE/'DESIGN.md'),
    ...
    model,tokenizer,model_path,args=legacy.load_model(argparse.Namespace(run_dir=j['checkpoint'],model_path=None,affine_ablation='none',device='cuda'))
    assert args['variant']==variant(j) and args['seed']==j['seed']
    dev=rows(HERE/'data/dev.jsonl');dev=dev[:8] if a.smoke else dev
    if not (out/'internal_dev_ce.json').exists():
        write(out/'internal_dev_ce.json',legacy.evaluate(model,tokenizer,dev,1,2048,torch.device('cuda')))
    data=rows(HERE/f"data/cmrc_{'dev' if a.smoke else 'eval'}.jsonl");data=data[:8] if a.smoke else data
    ...
    got=likelihood(model,tokenizer,'cmrc',data,out,a.smoke)
    generated=generation(model,tokenizer,data,out,eos,a.smoke)
    report=task_summary('cmrc',data,got,generated)
    report['source_sha256']={str(out/n):sha(out/n) for n in ['cmrc_likelihood.jsonl','cmrc_generation.jsonl']}
    write(out/'cmrc_scores.json',report);print(report['metrics'],flush=True)
    assert checkpoint_hashes(j['checkpoint'])==hashes
