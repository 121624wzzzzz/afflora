"""Render the final interpretation from audited, complete results only."""
import numpy as np
from common import HERE, now, read, sha, write


def main():
    d = read(HERE / 'RESULTS.json')
    audit = read(HERE / 'FINAL_AUDIT.json')
    assert d['complete'] and audit['status'] == 'passed'
    assert sha(HERE / 'RESULTS.json') == audit['source_sha256'][str(HERE / 'RESULTS.json')]
    models = [('qwen3_06b_chat', 'Qwen3-0.6B'), ('qwen25_15b_chat', 'Qwen2.5-1.5B-Instruct')]
    arms = [('reference', '原始聊天模型'), ('none', 'hidden LoRA'), ('output', 'hidden + output'),
            ('both', 'hidden + input + output'), ('hidden_budget', '等参数 hidden 对照')]

    def records(model, arm):
        return [r for r in d['records'] if r['model'] == model and r['arm'] == arm]

    def mean(model, arm, key, scale=1):
        return float(np.mean([r[key] for r in records(model, arm)])) * scale
    ...
    lines = [
        ...
        '', f'完成于 {now()}。本轮 24 组全新 CMRC 训练及评测全部完成，最终审计通过。此前中文迁移阶段的 26 个端点、52 项任务评测也已完成。全部实验臂、种子和正负结果保留。',
    ...
    for model, label in models:
        for arm, arm_label in arms:
            lines.append(f'| {label} | {arm_label} | {len(records(model, arm))} | {mean(model, arm, "em", 100):.4f} | {mean(model, arm, "f1", 100):.4f} | {mean(model, arm, "avg", 100):.4f} |')
    lines += ['', '**双边叠加的配对增量。** 下表均为名义 95% 区间。种子 t 区间描述固定评测集上三次训练的波动；篇章簇 bootstrap 区间以这三个已拟合模型为条件，描述评测篇章采样的不确定性。二者不能相互替代。',
              '', '| 模型 | 对照 | 平均增量 pp | 三种子 t 区间 | 篇章簇 bootstrap 区间 |',
              '| --- | --- | ---: | --- | --- |']
    for c in d['primary_contrasts']:
        label = dict(models)[c['model']]
        control = 'hidden' if c['control'] == 'none' else '等参数 hidden'
        lines.append(f'| {label} | {control} | {c["mean_effect_pp"]:+.4f} | {ci(c["seed_ci95"])} | {ci(c["cluster_bootstrap_ci95"])} |')
    lines += ['', '| 模型 / 对照 | seed 42 | seed 43 | seed 44 |', '| --- | ---: | ---: | ---: |']
    for c in d['primary_contrasts']:
        values = ' | '.join(f'{r["effect_pp"]:+.4f}' for r in c['per_seed'])
        control = 'hidden' if c['control'] == 'none' else '等参数 hidden'
        lines.append(f'| {dict(models)[c["model"]]} / {control} | {values} |')
    lines += ['', '四个主要比较的 Bonferroni 同时区间也全部保留，避免只挑一个正向比较。',
              '', '| 模型 / 对照 | 校正后三种子区间 | 校正后篇章簇区间 |', '| --- | --- | --- |']
    for c in d['primary_contrasts']:
        control = 'hidden' if c['control'] == 'none' else '等参数 hidden'
        lines.append(f'| {dict(models)[c["model"]]} / {control} | {ci(c["seed_ci_bonferroni4"])} | {ci(c["cluster_bootstrap_ci_bonferroni4"])} |')
    all_seed_cross_zero = all(c['seed_ci95'][0] <= 0 <= c['seed_ci95'][1] for c in d['primary_contrasts'])
    if all_seed_cross_zero:
        lines += ['', '本轮四个主要比较的名义 95% 种子区间全部包含零。不能把较窄的条件篇章区间当作跨训练可重复性的替代证据；未检出稳定收益也不等于证明所有设置下效应为零。']
    lines += ['', '**绝对提升的归属。**']
    for model, label in models:
        ref, hidden, both = [mean(model, arm, 'avg', 100) for arm in ['reference', 'none', 'both']]
        lines += ['', f'{label} 从未做本轮任务训练的 {ref:.4f}，到 hidden LoRA 的 {hidden:.4f}、双边叠加的 {both:.4f}。其中 hidden 相对原始模型已提高 {hidden-ref:.4f} 个百分点；双边方法相对 hidden 的额外贡献只有 {both-hidden:+.4f} 个百分点。大幅绝对提升不能全部归给边界叠加。']
    lines += ['', '**辅助概率指标与输出长度。** Internal dev CE 是含答案结束符的 token 加权指标；public dev answer CE/top-1 只计算参考答案内容，先等权平均每题的三个参考，再平均题目。划分、监督范围和权重都不同，不能混算或把两者差值解释为过拟合幅度。',
              '', '| 模型 | 配置 | Internal dev CE | Public dev answer CE | Answer top-1 % | 输出 token 均值 | 截断率 % |',
              '| --- | --- | ---: | ---: | ---: | ---: | ---: |']
    for model, label in models:
        for arm, arm_label in arms[1:]:
            lines.append(f'| {label} | {arm_label} | {mean(model, arm, "internal_dev_ce"):.6f} | {mean(model, arm, "answer_ce"):.6f} | {mean(model, arm, "answer_top1", 100):.4f} | {mean(model, arm, "generated_tokens"):.3f} | {mean(model, arm, "hit_token_cap", 100):.4f} |')
    for model, label in models:
        for control in ['none', 'hidden_budget']:
            a = {r['seed']: r for r in records(model, 'both')}
            b = {r['seed']: r for r in records(model, control)}
            ce_d = [a[s]['answer_ce'] - b[s]['answer_ce'] for s in [42, 43, 44]]
            dev_d = [a[s]['internal_dev_ce'] - b[s]['internal_dev_ce'] for s in [42, 43, 44]]
            lines += ['', f'{label}，both−{control}：public-dev answer CE 逐种子差值为 ' + ' / '.join(f'{v:+.6f}' for v in ce_d) + '；internal-dev CE 为 ' + ' / '.join(f'{v:+.6f}' for v in dev_d) + '（CE 负值表示改善）。']
    trained = [r for r in d['records'] if r['arm'] != 'reference']
    if all(r['hit_token_cap'] == 0 for r in trained):
        lines += ['', '全部 24 个任务训练端点的生成截断率均为零，本轮观察到的波动不能归因于 256 token 上限截断。']
    lines += [
        '', '**与前两轮证据合并看。**',
        '', '| 证据 | 支持的结论 | 尚未支持的结论 |',
        '| --- | --- | --- |',
    ...
    source_paths = [HERE / 'RESULTS.json', HERE / 'FINAL_AUDIT.json', HERE / 'make_report.py',
                    HERE.parent / 'stacking_chinese_transfer_20260915/FINAL_INTERPRETATION_ZH.md',
                    HERE.parent / 'stacking_claim_checks_20260914/FINAL_INTERPRETATION_ZH.md']
    write(HERE / 'REPORT_SOURCES.json', {'source_sha256': {str(p): sha(p) for p in source_paths},
                                       'report_sha256': sha(HERE / 'FINAL_INTERPRETATION_ZH.md')})
    print('Wrote final interpretation', flush=True)
