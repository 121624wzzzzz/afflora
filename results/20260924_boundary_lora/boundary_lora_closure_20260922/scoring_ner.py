...
def entity_score(value,row):
    gold=row['gold_spans'];ps=Counter();pt=Counter();valid=value is not None
    for i,e in enumerate(value or []):
        item=isinstance(e,dict) and set(e)=={'type','text','occurrence'}
        typed=isinstance(e,dict) and isinstance(e.get('type'),str) and e['type'] in TYPES and isinstance(e.get('text'),str) and bool(e['text'])
        matches=[j for j in range(len(row['text'])) if row['text'].startswith(e['text'],j)] if typed else []
        span=item and typed and type(e['occurrence'])==int and 0<=e['occurrence']<len(matches)
        if span:start=matches[e['occurrence']];end=start+len(e['text'])-1
        valid=valid and span
        ps[(e['type'],start,end) if span else ('INVALID',i)]+=1
