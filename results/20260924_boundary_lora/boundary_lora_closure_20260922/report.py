...
for key,a in lookup.items():
 ...
 if stage not in ['common','confirmation'] or method!='affine' or rank!=16 or bias!=(place!='u'):continue
 for comparison,bkey in [('ordinary', (stage,model,task,'vocab',place,1,False)),('hidden',(stage,model,task,'none','none',0,False))]:
  ...
  contrasts.append(dict(stage=stage,model=model,task=task,placement=place,comparison=comparison,seeds=seeds,differences=ds,mean=mean,ci95=[mean-margin,mean+margin],p_unadjusted=pvalue,positive=sum(x>0 for x in ds),negative=sum(x<0 for x in ds)))
for stage,comparison,total in [('confirmation','ordinary',10),('confirmation','hidden',10)]:
...
subprocess.run([PYTHON,'-B',str(HERE/'baseline_contrasts.py')],env=dict(os.environ,PYTHONDONTWRITEBYTECODE='1'),check=True)
