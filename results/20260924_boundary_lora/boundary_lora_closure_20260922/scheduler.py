...
def add_confirmation():
 ...
 write(HERE/'SELECTION_FROZEN.json',dict(at=now(),criterion='mean dev primary over two fixed seeds; ties favor ratio closest to 1',candidates=[.0625,.25,1.,4.],selection=[dict(architecture=list(k),ratio=r,dev_scores=dict(groups[k])) for k,r in choices.items()],jobs=selected))
 for s in selected:
...
try:
 while pending or active or not confirmation_added:
  ...
  for gpu,mb in sorted(mem.items(),key=lambda x:-x[1]):
   ...
   eligible=[s for s in pending if architecture(s) not in blocked and mb>=required(s) and (s['smoke'] or (instrumentation_names.issubset(set(done)) and architecture(s) in passed))]
