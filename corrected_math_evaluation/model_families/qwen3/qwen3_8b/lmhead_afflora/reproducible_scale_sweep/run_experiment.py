...
MODEL = Path("/home/wz/projects/mypro/im_exp/models/Qwen3-8B-Base")
...
class Job:
    ...
    hidden_rank: int
    seed: int
    affine_scale: int | None

    @property
    def variant(self) -> str:
        return "hidden_lora" if self.affine_scale is None else "affine_lm_head_plus_hidden_lora"
...
def build_jobs() -> list[Job]:
    jobs: list[Job] = []
    for seed in SEEDS:
        jobs.append(Job(f"hidden_hr4_seed{seed}", 4, seed, None))
    for scale in (1, 2, 4, 8, 16):
        for seed in SEEDS:
            jobs.append(Job(f"lmhead_ar1_s{scale}_hr4_seed{seed}", 4, seed, scale))
    for seed in SEEDS:
        jobs.append(Job(f"hidden_hr8_seed{seed}", 8, seed, None))
        jobs.append(Job(f"lmhead_ar1_s8_hr8_seed{seed}", 8, seed, 8))
    return jobs
...
def train_command(job: Job) -> list[str]:
    ...
    if job.affine_scale is not None:
        cmd += ["--affine-rank", "1", "--affine-alpha", str(job.affine_scale)]
    return cmd
...
def eval_command(job: Job, task: str, shard: int) -> list[str]:
    ...
    return [
        ...
        "--batch-size", "64", "--max-new-tokens", "512",
...
def run_eval_task(job: Job, task: str) -> bool:
    ...
    env["DS_IGNORE_CUDA_DETECTION"] = "1"
    for attempt in (1, 2):
        update_job(job.name, status=f"evaluating_{task}", gpu="0-7", eval_attempt=attempt)
        event("eval_start", job=job.name, task=task, attempt=attempt)
        ...
        for shard, gpu in enumerate(GPUS):
            ...
            process = subprocess.Popen(eval_command(job, task, shard), cwd=ROOT, env=shard_env, stdout=log, stderr=subprocess.STDOUT)
        ...
        if all(code == 0 for code in codes):
            ...
            if merged.returncode == 0 and eval_complete(job, task):
                event("eval_finish", job=job.name, task=task, attempt=attempt)
                return True
        event("eval_failure", job=job.name, task=task, attempt=attempt, shard_returncodes=codes)
...
def config_label(job: Job) -> str:
    return f"hidden_hr{job.hidden_rank}" if job.affine_scale is None else f"lmhead_ar1_s{job.affine_scale}_hr{job.hidden_rank}"
...
def write_summary_locked() -> None:
    ...
    lines = [
        "# Reproducible AffLoRA sweep results", "", f"Updated: {now()}", "",
