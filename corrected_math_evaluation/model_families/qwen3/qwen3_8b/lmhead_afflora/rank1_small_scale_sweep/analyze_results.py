...
REFERENCE_OUTPUTS = HERE.parent / "reproducible_scale_sweep" / "outputs"
DESTINATION = HERE / "ANALYSIS.md"
SEEDS = (42, 43, 44)
T_975_DF2 = 4.3026527297
SMALL_SCALES = (0.0625, 0.125, 0.25, 0.5)
REFERENCE_SCALES = (1, 2, 4, 8, 16)
...
def configs() -> list[tuple[str, int, float | None]]:
    values = [("hidden_hr4", 4, None)]
    values += [(f"lmhead_ar1_s{scale_tag(scale)}_hr4", 4, scale) for scale in SMALL_SCALES]
    values += [(f"lmhead_ar1_s{scale}_hr4", 4, scale) for scale in REFERENCE_SCALES]
    return values
...
def read_score(label: str, seed: int, task: str) -> float | None:
    ...
    if not path.exists():
        path = REFERENCE_OUTPUTS / task / f"{run_name(label, seed)}_full.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    hr4_complete: list[tuple[float, float, float]] = []
    for scale in (*SMALL_SCALES, *REFERENCE_SCALES):
        label = f"lmhead_ar1_s{scale_tag(scale)}_hr4" if scale in SMALL_SCALES else f"lmhead_ar1_s{scale}_hr4"
        ms = [score[label, seed, "math"] for seed in SEEDS if (label, seed, "math") in score]
        gs = [score[label, seed, "gsm8k"] for seed in SEEDS if (label, seed, "gsm8k") in score]
        if len(ms) == len(gs) == 3:
            hr4_complete.append((scale, statistics.mean(ms), statistics.mean(gs)))
    lines += ["", "## Scale sweep ranking", ""]
    if len(hr4_complete) == len(SMALL_SCALES) + len(REFERENCE_SCALES):
        ...
        for rank, (scale, m, g) in enumerate(sorted(hr4_complete, key=lambda x: x[1], reverse=True), 1):
            lines.append(f"| {rank} | {scale:g} | {m:.4f}% | {g:.4f}% |")
    else:
        total = len(SMALL_SCALES) + len(REFERENCE_SCALES)
        lines.append(f"Scale ranking pending: {len(hr4_complete)}/{total} configurations have all three seeds and both evaluations.")
