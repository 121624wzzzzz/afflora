...
def build_jobs() -> list[Job]:
    ...
    for scale in (0.0625, 0.125, 0.25, 0.5):
        for seed in SEEDS:
            jobs.append(Job(f"lmhead_ar1_s{scale_tag(scale)}_hr4_seed{seed}", 4, seed, scale))
    return jobs
...
def config_label(job: Job) -> str:
    return f"hidden_hr{job.hidden_rank}" if job.affine_scale is None else f"lmhead_ar1_s{scale_tag(job.affine_scale)}_hr{job.hidden_rank}"
...
def write_summary_locked() -> None:
    ...
    lines = [
        "# AffLoRA small-scale sweep results", "", f"Updated: {now()}", "",
