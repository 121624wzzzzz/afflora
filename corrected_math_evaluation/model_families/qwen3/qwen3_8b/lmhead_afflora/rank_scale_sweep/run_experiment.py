...
SEEDS = (42, 43, 44)
GPUS = tuple(range(8))
EVAL_BATCH_CANDIDATES = (128, 96, 64)
...
def build_jobs() -> list[Job]:
    ...
    for rank in (2, 4, 8, 16):
        for scale in (0.0625, 0.125, 0.25, 0.5):
            for seed in SEEDS:
                jobs.append(Job(f"lmhead_ar{rank}_s{scale_tag(scale)}_hr4_seed{seed}", 4, seed, rank, scale))
    return jobs
...
def train_command(job: Job) -> list[str]:
    ...
    if job.affine_scale is not None:
        ...
        cmd += ["--affine-rank", str(job.affine_rank), "--affine-alpha", f"{affine_alpha:g}"]
    return cmd
...
def config_label(job: Job) -> str:
    return f"hidden_hr{job.hidden_rank}" if job.affine_scale is None else f"lmhead_ar{job.affine_rank}_s{scale_tag(job.affine_scale)}_hr{job.hidden_rank}"
...
def write_summary_locked() -> None:
    ...
    lines = [
        "# AffLoRA rank-scale sweep results", "", f"Updated: {now()}", "",
