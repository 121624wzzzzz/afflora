...
REFERENCE_OUTPUTS = [
    HERE.parent / "rank1_small_scale_sweep" / "outputs",
    HERE.parent / "reproducible_scale_sweep" / "outputs",
]
DESTINATION = HERE / "ANALYSIS.md"
SEEDS = (42, 43, 44)
T_975_DF2 = 4.3026527297
SMALL_SCALES = (0.0625, 0.125, 0.25, 0.5)
RANKS = (1, 2, 4, 8, 16)
...
def configs() -> list[tuple[str, int, int | None, float | None]]:
    values = [("hidden_hr4", 4, None, None)]
    for rank in RANKS:
        values += [(f"lmhead_ar{rank}_s{scale_tag(scale)}_hr4", 4, rank, scale) for scale in SMALL_SCALES]
    return values
...
def read_score(label: str, seed: int, task: str) -> float | None:
    ...
    if not path.exists():
        for root in REFERENCE_OUTPUTS:
            candidate = root / task / f"{run_name(label, seed)}_full.json"
            if candidate.exists():
                path = candidate
                break
    if not path.exists():
...
def main() -> None:
    ...
    for label, hidden_rank, _, scale in configs():
        if scale is None:
            continue
    ...
    complete: list[tuple[int, float, float, float]] = []
    for rank in RANKS:
        for scale in SMALL_SCALES:
            label = f"lmhead_ar{rank}_s{scale_tag(scale)}_hr4"
            ms = [score[label, seed, "math"] for seed in SEEDS if (label, seed, "math") in score]
            gs = [score[label, seed, "gsm8k"] for seed in SEEDS if (label, seed, "gsm8k") in score]
            if len(ms) == len(gs) == 3:
                complete.append((rank, scale, statistics.mean(ms), statistics.mean(gs)))
    lines += ["", "## Rank-scale ranking", ""]
    if len(complete) == len(RANKS) * len(SMALL_SCALES):
        lines += ["| rank by MATH | affine rank | scale | MATH mean | GSM8K mean |", "| ---: | ---: | ---: | ---: | ---: |"]
        for idx, (rank, scale, m, g) in enumerate(sorted(complete, key=lambda x: x[2], reverse=True), 1):
            lines.append(f"| {idx} | {rank} | {scale:g} | {m:.4f}% | {g:.4f}% |")
    else:
        total = len(RANKS) * len(SMALL_SCALES)
        lines.append(f"Rank-scale ranking pending: {len(complete)}/{total} configurations have all three seeds and both evaluations.")
