...
LOGS = HERE / "logs"
STATE_FILE = HERE / "state_mergeable_rank.json"
EVENTS_FILE = HERE / "events_mergeable_rank.jsonl"
RESULTS_FILE = HERE / "RESULTS_MERGEABLE_RANK.md"
...
class Job:
    ...
    def affine_alpha(self) -> float:
        return self.affine_scale * self.affine_rank
...
JOBS = [Job(model_alias, rank) for model_alias in MODELS for rank in AFFINE_RANKS]
...
def result_for_name(name: str, task: str) -> float | None:
...
def result_for(job: Job, task: str) -> float | None:
    if not eval_complete(job, task):
        return None
    return result_for_name(job.name, task)
...
def write_summary_locked() -> None:
    ...
    for job in JOBS:
        ...
        bm = result_for_name(job.baseline_name, "math")
        bg = result_for_name(job.baseline_name, "gsm8k")
        if None not in (m, g, bm, bg):
            lines.append(f"| {job.model_alias} | {job.hidden_rank} | {job.affine_rank} | {job.seed} | {m:.4f}% | {m-bm:+.4f} pp | {g:.4f}% | {g-bg:+.4f} pp |")
        else:
            lines.append(f"| {job.model_alias} | {job.hidden_rank} | {job.affine_rank} | {job.seed} | {'-' if m is None else f'{m:.4f}%'} | - | {'-' if g is None else f'{g:.4f}%'} | - |")
