...
MODELS = {
    "qwen25_05b": Path("/home/wz/projects/mypro/im_exp/models/Qwen2.5-0.5B-Base"),
    "qwen25_15b": Path("/home/wz/projects/mypro/im_exp/models/Qwen2.5-1.5B-Base"),
}
...
class Job:
    ...
    def variant(self) -> str:
        ...
        if self.family == "emb":
            return "affine_input"
        if self.family == "mergeable":
            return "affine_input_lm_head"
...
def build_jobs() -> list[Job]:
    ...
    for model_alias in MODELS:
        jobs.append(Job(f"{model_alias}_base", model_alias, "base", None, 0, None))
        scale = MODEL_SCALE[model_alias]
        for family in ("emb", "mergeable"):
            for affine_rank in AFFINE_RANKS:
                for seed in SEEDS:
                    jobs.append(
                        Job(
                            f"{model_alias}_{family}_ar{affine_rank}_s{scale_tag(scale)}_seed{seed}",
                            model_alias,
                            family,
                            seed,
                            affine_rank,
...
def train_command(job: Job) -> list[str]:
    if job.family == "base":
        raise ValueError("Base jobs are eval-only.")
    if job.seed is None or job.affine_scale is None:
        raise ValueError(f"Training job is missing seed/scale: {job}")
    cmd = [
        ...
        "--save-strategy", "no", "--save-total-limit", "1",
...
def run_training(job: Job, gpu: int) -> bool:
    ...
    env["HF_HOME"] = str(HF_CACHE / "home")
    env["HF_DATASETS_CACHE"] = str(HF_CACHE / "datasets")
    env["XDG_CACHE_HOME"] = str(HF_CACHE / "xdg")
    for attempt in (1, 2):
...
def eval_command(job: Job, task: str, shard: int, batch_size: int) -> list[str]:
    ...
    if job.family != "base":
        cmd[4:4] = ["--run-dir", str(CHECKPOINTS / job.name)]
    return cmd
...
def config_label(job: Job) -> str:
    if job.family == "base":
        return f"{job.model_alias}_base"
    return f"{job.model_alias}_{job.family}_ar{job.affine_rank}_s{scale_tag(job.affine_scale or 0)}"
...
def write_summary_locked() -> None:
    ...
    lines = [
        "# Small-model affine-only AffLoRA math sweep results", "", f"Updated: {now()}", "",
...
def main() -> None:
    for directory in (CHECKPOINTS, OUTPUTS / "math", OUTPUTS / "gsm8k", LOGS / "train", LOGS / "math", LOGS / "gsm8k", HF_CACHE):
