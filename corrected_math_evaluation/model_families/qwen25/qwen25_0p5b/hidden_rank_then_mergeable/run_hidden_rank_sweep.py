...
class Job:
    ...
    @property
    def name(self) -> str:
        return f"qwen25_05b_hidden_hr{self.hidden_rank}_seed{self.seed}"
...
JOBS = [Job(rank, seed) for rank in HIDDEN_RANKS for seed in SEEDS]
...
def write_summary_locked() -> None:
    ...
    lines += ["", "## Per-run results", "", "| hidden rank | seed | MATH clean | GSM8K |", "| ---: | ---: | ---: | ---: |"]
    for job in JOBS:
        m = result_for(job, "math")
        g = result_for(job, "gsm8k")
        lines.append(
            f"| {job.hidden_rank} | {job.seed} | {m:.4f}% | {g:.4f}% |"
            if m is not None and g is not None
            else f"| {job.hidden_rank} | {job.seed} | {'-' if m is None else f'{m:.4f}%'} | {'-' if g is None else f'{g:.4f}%'} |"
        )
    lines += ["", "## Three-seed aggregates", "", "| hidden rank | completed seeds | MATH mean ± sd | GSM8K mean ± sd |", "| ---: | ---: | ---: | ---: |"]
    for rank in HIDDEN_RANKS:
        jobs = [job for job in JOBS if job.hidden_rank == rank]
