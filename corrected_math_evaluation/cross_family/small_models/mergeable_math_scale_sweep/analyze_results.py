...
SEEDS = (42, 43, 44)
MODELS = ("qwen3_06b", "qwen25_05b", "qwen25_15b")
SCALES = (0.25, 1.0, 4.0, 8.0)
...
def configs() -> list[tuple[str, str, float | None]]:
    values: list[tuple[str, str, float | None]] = []
    for model in MODELS:
        values.append((f"{model}_hidden_hr4", model, None))
        for scale in SCALES:
            values.append((f"{model}_mergeable_ar16_s{scale_tag(scale)}_hr4", model, scale))
    return values
...
def main() -> None:
    ...
    ranking: list[tuple[str, str, float, float, float]] = []
    for label, model, scale in configs():
        if scale is None:
            continue
        ...
        gs = [score[label, seed, "gsm8k"] for seed in SEEDS if (label, seed, "gsm8k") in score]
        if len(ms) == len(gs) == 3:
            ranking.append((model, f"{scale:g}", statistics.mean(ms), statistics.mean(gs), statistics.mean(md) if len(md) == 3 else float("nan")))
    ...
    for model in MODELS:
        ...
        if not rows:
            ...
            lines.append("")
            continue
        lines += ["| rank by MATH | scale | MATH mean | GSM8K mean | Δ MATH vs hidden |", "| ---: | ---: | ---: | ---: | ---: |"]
        for idx, (_, scale, m, g, dm) in enumerate(sorted(rows, key=lambda x: x[2], reverse=True), 1):
            lines.append(f"| {idx} | {scale} | {m:.4f}% | {g:.4f}% | {dm:+.4f} pp |")
        lines.append("")
